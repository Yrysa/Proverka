...
$State = [ordered]@{
    ...
    Counters = [ordered]@{
        Roots=0; Files=0; Processes=0; Java=0; Modules=0; Minecraft=0; Prefetch=0; Recent=0
        Services=0; Tasks=0; Registry=0; Drivers=0; Network=0; Usb=0; Wmi=0; Traces=0; Errors=0
    }
}

$Ansi = [ordered]@{ Reset="`e[0m"; Red="`e[91m"; Green="`e[92m"; Yellow="`e[93m"; Blue="`e[94m"; Purple="`e[95m"; Cyan="`e[96m"; White="`e[97m"; Gray="`e[90m" }
if ($NoColor -or $Host.Name -match "ISE") { foreach ($k in @($Ansi.Keys)) { $Ansi[$k] = "" } }

function ColorText([string]$Text, [string]$Color) { return "$($Ansi[$Color])$Text$($Ansi.Reset)" }
function FitText([string]$Text, [int]$Size) { if ($null -eq $Text) { $Text = "" }; $x = $Text -replace "`r|`n", " "; if ($x.Length -gt $Size) { return $x.Substring(0, [Math]::Max(0, $Size - 3)) + "..." }; return $x.PadRight($Size) }
function PrintLine([string]$Color = "Purple") { Write-Host (ColorText ("=" * $App.Width) $Color) }

function Panel([string]$Title, [string[]]$Rows, [string]$Color = "Cyan") {
    $w = $App.Width
    Write-Host (ColorText ("+" + ("-" * ($w - 2)) + "+") $Color)
    Write-Host (ColorText "|" $Color) -NoNewline; Write-Host (ColorText (FitText ("  " + $Title) ($w - 2)) "White") -NoNewline; Write-Host (ColorText "|" $Color)
    Write-Host (ColorText ("+" + ("-" * ($w - 2)) + "+") $Color)
    foreach ($row in $Rows) { Write-Host (ColorText "|" $Color) -NoNewline; Write-Host (FitText ("  " + $row) ($w - 2)) -NoNewline; Write-Host (ColorText "|" $Color) }
    Write-Host (ColorText ("+" + ("-" * ($w - 2)) + "+") $Color)
}
...
function Banner {
    Clear-Host
    PrintLine "Purple"
    Write-Host (ColorText "  PROVERKA - ADVANCED MINECRAFT FORENSIC SCANNER" "Cyan")
    Write-Host (ColorText ("  Version: " + $App.Version + " | UI: ASCII SAFE | PowerShell 5.1 compatible") "White")
    PrintLine "Purple"
}
...
function Status([string]$Stage, [string]$Text, [int]$Percent = -1) {
    ...
    if ($Percent -ge 0) {
        ...
        Write-Host (ColorText ("> [$bar] $($Percent.ToString().PadLeft(3))% ") "Cyan") -NoNewline
        Write-Host (FitText "$Stage $Text" ([Math]::Max(20, $App.Width - 30)))
    } else {
        Write-Host (ColorText "> " "Cyan") -NoNewline; Write-Host "$Stage $Text"
    }
}
...
function Get-Tokens {
    $base = @(
        "vape","raven","rise","drip","entropy","whiteout","slinky","liquidbounce","wurst","meteor","impact","future","rusherhack","phobos","konas","pyro","boze","prestige","thunderhack","bleachhack","forgehax","inertia","sigma","flux","tenacity","zeroday","astolfo","fdp","dope","koid","iridium","autoclicker","clicker","injector","loader","kdmapper","drvmap","spoofer","ghostclient","reach","velocity","aimassist","xray","baritone","killaura","antikb","antiknockback","timer","hitbox","esp","tracers","wallhack","triggerbot","macro","jvmti","agent","nativehook","rawinput","interception","mapper","unsigned","mixin","fabricloader","forgeclient","ghost","silent","selfdestruct","destruct","cleaner","bypass","dll","dlc","client","pulse"
    )
    $extra = @()
...
function Score-Item([string]$Name, [string]$Path, [string[]]$Words) {
    ...
    $hay = (($Name + " " + $Path).ToLowerInvariant())
...
function Score-Text([string]$Text, [string[]]$Words) {
    $score = 0
    $reasons = New-Object 'System.Collections.Generic.List[string]'
    $hay = ([string]$Text).ToLowerInvariant()
...
function Get-Roots {
    ...
    $paths = @(
        (Join-Path $home "Downloads"), (Join-Path $home "Desktop"), (Join-Path $home "Documents"),
        (Join-Path $home "AppData\Roaming\.minecraft"), (Join-Path $home "AppData\Roaming\.tlauncher"),
        (Join-Path $home "AppData\Roaming\PrismLauncher"), (Join-Path $home "AppData\Roaming\PolyMC"),
        (Join-Path $home "AppData\Roaming\MultiMC"), (Join-Path $home "AppData\Roaming\ModrinthApp"),
        (Join-Path $home "AppData\Roaming\ATLauncher"), (Join-Path $home "AppData\Local\Temp"),
        (Join-Path $env:PROGRAMDATA ""), (Join-Path $env:APPDATA "Discord")
    )
...
function Scan-MinecraftInventory([string[]]$Words) {
    ...
    $home = [Environment]::GetFolderPath("UserProfile")
    $targets = @(
        (Join-Path $home "AppData\Roaming\.minecraft\mods"), (Join-Path $home "AppData\Roaming\.minecraft\versions"),
        (Join-Path $home "AppData\Roaming\.minecraft\libraries"), (Join-Path $home "AppData\Roaming\.minecraft\config"),
        (Join-Path $home "AppData\Roaming\PrismLauncher\instances"), (Join-Path $home "AppData\Roaming\PolyMC\instances"),
        (Join-Path $home "AppData\Roaming\MultiMC\instances"), (Join-Path $home "AppData\Roaming\ModrinthApp\profiles"),
        (Join-Path $home "AppData\Roaming\.tlauncher"), (Join-Path $home "AppData\Roaming\ATLauncher\instances")
    )
...
function Scan-Drivers([string[]]$Words) {
    if (-not $Drivers -and -not $Forensic -and -not $FullSystem) { return }
    Status "KERNEL" "drivers" 74
...
function Scan-Network {
    ...
    Status "TRACE" "network cache" 80
    SafeRun { ipconfig /displaydns 2>$null | Select-String -Pattern "Record Name" | ForEach-Object { $State.Counters.Network++; $line=$_.Line.Trim(); if ($line -match "pastebin|githubusercontent|cdn.discordapp|anonfiles|gofile|mediafire|workupload|mega|dropbox|pixeldrain") { Add-Finding "dns" $line "" 32 @("download-domain") "" } } } "dns"
}
...
function Show-Results {
    $f = @($State.Findings | Sort-Object score -Descending | Select-Object -First $Top)
    $critical = @($f | Where-Object { $_.score -ge 70 }).Count
    $medium = @($f | Where-Object { $_.score -ge 40 -and $_.score -lt 70 }).Count
    $low = @($f | Where-Object { $_.score -lt 40 }).Count
    Panel "SUMMARY" @(
        "Version: $($App.Version)",
        "Findings: $($f.Count) | Detected: $critical | Suspicious: $medium | Trace: $low",
        "Live Java: $($State.Counters.Java) | Modules: $($State.Counters.Modules) | Minecraft items: $($State.Counters.Minecraft)",
        "Files: $($State.Counters.Files) | Prefetch: $($State.Counters.Prefetch) | Recent: $($State.Counters.Recent)",
        "Report: $($App.Report)"
    ) "Purple"
    if ($f.Count -gt 0) {
        $rows = @($f | Select-Object -First ([Math]::Min(16, $f.Count)) | ForEach-Object { ("[{0}] {1,3} | {2} | {3} | {4}" -f (Verdict $_.score), $_.score, $_.type, $_.name, ($_.reasons -join ",")) })
        Panel "TOP FINDINGS" $rows "Yellow"
    } else { Panel "RESULT" @("No suspicious items reached the selected threshold.") "Green" }
}

function Self-Test { Banner; Panel "SELF TEST" @("PowerShell: $($PSVersionTable.PSVersion)", "Build: $($App.Version)", "Report path: $($App.Report)", "ASCII safe UI: true", "Width: $($App.Width)") "Green" }
...
function Main {
    ...
    $words = Get-Tokens
    Panel "RUN CONFIG" @(
        "Mode: $(if($FullSystem){"full system"}elseif($Deep){"deep"}elseif($Fast){"fast"}else{"standard + live forensic"})",
        "Forensic: $([bool]$Forensic) | Drivers: $([bool]$Drivers) | VM: $([bool]$VMCheck)",
        "Max candidates: $MaxCandidates | Time limit: $MaxMinutes min | Threshold: $MinScore",
        "Rule tokens: $($words.Count)"
    ) "Cyan"
    if (-not $NoPrompt) { Write-Host (ColorText "Press Enter to start scan..." "Gray") -NoNewline; [void][Console]::ReadLine() }
