...
function Write-UiText {
    ...
    if ($Quiet) { return }
    try {
        if ($NoNewLine) { Write-Host $Text -ForegroundColor $Foreground -NoNewline }
...
function Write-UiBox {
    ...
    $inner = $Width - 2
    Write-UiText ("+" + ("-" * $inner) + "+") $BorderColor
    Write-UiText "|" $BorderColor -NoNewLine
    Write-UiText (("  " + $Title).PadRight($inner)) $TitleColor -NoNewLine
    Write-UiText "|" $BorderColor
    Write-UiText ("+" + ("-" * $inner) + "+") $BorderColor

    foreach ($line in $Lines) {
        $safe = [string]$line
        if ($safe.Length -gt ($inner - 2)) { $safe = $safe.Substring(0, $inner - 5) + "..." }
        Write-UiText "|" $BorderColor -NoNewLine
        Write-UiText (("  " + $safe).PadRight($inner)) ([ConsoleColor]::Gray) -NoNewLine
        Write-UiText "|" $BorderColor
    }
...
function Show-StageProgress {
    ...
    $percent = 0
    if ($Total -gt 0) { $percent = [int](($Current / [double]$Total) * 100) }
    $barSize = 34
    $done = [Math]::Min($barSize, [Math]::Max(0, [int](($percent / 100.0) * $barSize)))
    $bar = ("#" * $done) + ("." * ($barSize - $done))

    Write-UiText ""
    Write-UiText ("> [{0}] {1,3}%  STEP {2}/{3}" -f $bar, $percent, $Current, $Total) Cyan
    Write-UiText ("  " + $Title) White
    if (-not [string]::IsNullOrWhiteSpace($Description)) {
        Write-UiText ("  " + $Description) DarkGray
    }
}
...
function Invoke-UiStage {
    ...
    Show-StageProgress -Current $Current -Total $Total -Title $Title -Description $Description
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    try {
        & $Action
        $sw.Stop()
        if (-not $Quiet) {
            Write-UiText ("  OK: {0} completed in {1:n1}s" -f $Title, $sw.Elapsed.TotalSeconds) Green
        }
    } catch {
        $sw.Stop()
        Write-Log 'ERROR' ("Этап '{0}' завершился ошибкой: {1}" -f $Title, $_.Exception.Message)
        throw
    }
}

function Show-RunOverview {
    $mode = if ($Deep) { 'DEEP' } elseif ($OnlyMinecraft) { 'ONLY MINECRAFT' } else { 'FULL AUTO' }
    $html = if ($NoHtml) { 'OFF' } else { 'ON' }
    Write-UiBox -Title "RUN CONFIG" -Lines @(
        "Mode: $mode",
        "Output: $script:TempRoot",
        "HTML report: $html | Open report: $OpenReport",
        "MaxJarMB: $MaxJarMB | MaxEntryMB: $MaxEntryMB",
        "Extra paths: $($ExtraScanPath.Count)"
    ) -BorderColor DarkRed -TitleColor White -Width 100
}

function Show-FinalUiSummary {
    if ($Quiet) { return }
    $verdict = Get-VerdictObject
    $color = [ConsoleColor]::Green
    if ($verdict.Status -eq 'CHEAT_LIKELY') { $color = [ConsoleColor]::Red }
    elseif ($verdict.Status -like 'SUSPICIOUS*') { $color = [ConsoleColor]::Yellow }

    Write-UiText ""
    Write-UiBox -Title "FINAL SUMMARY" -Lines @(
        "Verdict: $($verdict.Status)",
        "Title: $($verdict.Title)",
        "Score: $($verdict.Score)",
        "Critical: $($verdict.Critical) | High: $($verdict.High) | Medium: $($verdict.Medium) | Low: $($verdict.Low)",
        "Scanned files: $script:ScannedFiles | JAR/ZIP: $script:ScannedJars | Read errors: $script:ReadErrors",
        "TXT: $script:ReportFile",
        "JSON: $script:JsonReport",
        "HTML: $script:HtmlReport"
    ) -BorderColor $color -TitleColor $color -Width 100
}
...
function Start-FullAnalysis {
    ...
    $steps.Add([pscustomobject]@{
        Title = 'Инициализация базы индикаторов'
        Description = 'Загрузка встроенной и внешней БД сигнатур.'
        Action = { Initialize-CheatDB }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Сводка системы'
        Description = 'ОС, пользователь, права администратора, версия PowerShell.'
        Action = { Invoke-SystemSummary }
    }) | Out-Null

    if (-not $OnlyMinecraft) {
        $steps.Add([pscustomobject]@{
            Title = 'Процессы, окна и командные строки'
            Description = 'Поиск активных клиентов, кликеров и подозрительных Java-аргументов.'
            Action = { Invoke-ProcessAndWindowCheck }
        }) | Out-Null
        $steps.Add([pscustomobject]@{
            Title = 'Автозагрузка и задачи планировщика'
            Description = 'Проверка Startup, Run-ключей и Scheduled Tasks.'
            Action = { Invoke-StartupAndTaskCheck }
        }) | Out-Null
        $steps.Add([pscustomobject]@{
            Title = 'Hosts и Prefetch'
            Description = 'Следы запусков и домены, связанные с чит-клиентами.'
            Action = { Invoke-HostsAndPrefetchCheck }
        }) | Out-Null
    } else {
        $steps.Add([pscustomobject]@{
            Title = 'OnlyMinecraft: процессы и окна'
            Description = 'Системные автозагрузки частично пропущены.'
            Action = {
    ...
    $steps.Add([pscustomobject]@{
        Title = 'Java / Minecraft процессы и JAR'
        Description = 'Командные строки, javaagent, JAR-пути и DLL-модули.'
        Action = { Invoke-MinecraftProcessCheck }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Minecraft папки, моды и библиотеки'
        Description = 'mods, versions, libraries, config и популярные лаунчеры.'
        Action = { Invoke-MinecraftFolderCheck }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Сетевые подключения'
        Description = 'Сравнение активных подключений с известными доменами/адресами.'
        Action = { Invoke-NetworkCheck }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Deep sweep'
        Description = 'Дополнительный поиск свежих подозрительных файлов, если включен -Deep.'
        Action = { Invoke-DeepSweep }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Сохранение JSON отчёта'
        Description = 'Структурированный отчёт для последующей проверки.'
        Action = { Save-JsonReport }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Создание HTML отчёта'
        Description = 'Красный премиальный отчёт с таблицей находок.'
        Action = { Save-HtmlReport }
    }) | Out-Null
    $steps.Add([pscustomobject]@{
        Title = 'Финальный вердикт'
        Description = 'Подсчёт score и вывод рекомендаций.'
        Action = { Write-FinalVerdict }
    }) | Out-Null

    $total = $steps.Count
    for ($i = 0; $i -lt $total; $i++) {
        $stage = $steps[$i]
        Invoke-UiStage -Current ($i + 1) -Total $total -Title $stage.Title -Description $stage.Description -Action $stage.Action
    }
